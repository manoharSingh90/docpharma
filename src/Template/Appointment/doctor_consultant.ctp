...
<?php
echo $this->Html->script(['pharmacy/moment.min.js','pharmacy/bootstrap-datetimepicker.js','pharmacy/bootstrap-multiselect.js','jquery.MultiFile-minified.js','pharmacy/selectize.js']);
?>
...
$('body').on('click','.donePopupProduct',function()
{
	if($('.pickMed').is(':checked'))
	{
		...
		if($(".product_name").val()!="") //GET COMPANY NAME
		{
			var request = "<Request><Detail><Product reference='"+product_guid[0]+"'><Items/><Packages/><TherapeuticClasses/><ATCCodes/><Companies/><Documents/></Product></Detail></Request>";
			
			$.ajax({
				url: '<?php echo $this->Url->build(["controller"=>"Appointment", "action"=>"getCompanyFormName"]); ?>',
				type: "POST",
...
{
	var id = $(".product_name").val().split('_');
	$('#product_name').selectize();
	if($("input[name='product_search_by']:checked").val()==2 && id[0]!="") //TO GET BRAND FROM MOLECULE
	{
		var request = "<Request><List><Product><ByMolecules combination='XAND'><ByMolecule reference='"+id[0]+"'/></ByMolecules></Product></List></Request>";
		
		$.ajax({
			url: '<?php echo $this->Url->build(["controller"=>"Appointment", "action"=>"getBrandsFromMolecule"]); ?>',
			type: "POST",
			data: {'CIMS_Request' : request},
			headers:
			{
	...
	if($(".product_name").val()!="") //GET COMPANY NAME
	{
		var request = "<Request><Detail><Product reference='"+id[0]+"'><Items/><Packages/><TherapeuticClasses/><ATCCodes/><Companies/><Documents/></Product></Detail></Request>";
